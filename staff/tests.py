...
class StaffManagementApiTests(APITestCase):
    ...
    def test_non_admin_forbidden_list_create_update_delete(self):
        ...
        res = self.client.patch(
            f"/api/admin/staff/{self.staff_user.pk}/",
            {"first_name": "Updated"},
            format="json",
        )
        self.assertEqual(res.status_code, 403, res.data)
        ...
        # Delete
        res = self.client.delete(f"/api/admin/staff/{self.staff_user.pk}/")
        self.assertEqual(res.status_code, 403, res.data)
    ...
    def test_list_search_filter(self):
        ...
        res = self.client.get("/api/admin/staff/?search=searchtest")
        self.assertEqual(res.status_code, 200, res.data)
    ...
    def test_list_is_active_filter(self):
        ...
        res = self.client.get("/api/admin/staff/?is_active=true")
        self.assertEqual(res.status_code, 200, res.data)
        for result in res.data["results"]:
            self.assertTrue(result["is_active"])

        # Filter for inactive only
        res = self.client.get("/api/admin/staff/?is_active=false")
        self.assertEqual(res.status_code, 200, res.data)
        self.assertGreaterEqual(res.data["count"], 1)
        for result in res.data["results"]:
    ...
    def test_retrieve_staff_user(self):
        ...
        res = self.client.get(f"/api/admin/staff/{self.staff_user.pk}/")
        self.assertEqual(res.status_code, 200, res.data)
