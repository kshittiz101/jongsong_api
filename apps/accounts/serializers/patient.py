...
class PatientProfileAdminPatchSerializer(serializers.Serializer):
    """Admin partial update of linked user + PatientProfile (user fields + clinical profile)."""
    ...
    def update(self, profile: PatientProfile, validated_data):
        ...
        for key in _PROFILE_ADMIN_PATCHABLE:
            if key in validated_data:
                setattr(profile, key, validated_data[key])
        if any(k in validated_data for k in _PROFILE_ADMIN_PATCHABLE):
            try:
                profile.full_clean()
            except DjangoValidationError as exc:
                raise serializers.ValidationError(exc.message_dict) from exc
